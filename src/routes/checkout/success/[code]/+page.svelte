<script lang="ts">
	...
	import { PUBLIC_SITE_EMAIL } from '$env/static/public';
			...
			<section class="flex-auto px-4 pb-16 pt-12 sm:px-6 sm:pt-16 lg:px-8 lg:pb-4 lg:pt-0">
				<div class="mx-auto max-w-lg">
									...
									<div class="flex-1">
										...
										<div class="mt-2 flex justify-between">
											...
											<div class="font-medium">
												{formatCurrency(line.linePriceWithTax, data.channel.currencyCode)}
											</div>
										</div>
									</div>
								</div>
					...
					<div class="mt-6 flex justify-between text-lg font-medium">
						<span>{m.total()}</span>
						<span>{formatCurrency(order.totalWithTax, data.channel.currencyCode)}</span>
					</div>
