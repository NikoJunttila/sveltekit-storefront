import { createClient, GetCollections, GetTopLevelCollections, ActiveChannel } from '$lib/vendure';
...
export async function load() {
	return {
		...
		topCollections: await client.query(GetTopLevelCollections, {}).toPromise().then((result : any) => result?.data?.collections?.items),
		 channel: await client.query(ActiveChannel, {}).toPromise().then((result : any) => result?.data?.activeChannel),
	};
}
