			...
			>
				...
				<div class=" flex w-full flex-1 flex-col p-4 bg-primary-200 dark:bg-gray-800">
					<div class="mb-4 flex flex-col gap-2">
						...
						<p class="text-xl font-bold text-primary-600">
							{#if p.price.__typename === 'PriceRange'}
								{formatCurrency(p.price.min, p.currencyCode)}
							{:else if p.price.__typename === 'SinglePrice'}
								{formatCurrency(p.price.value, p.currencyCode)}
							{/if}
						</p>
					</div>
					<div class="mt-auto grid w-full">
						{#if p.inStock}
							<button
								class="flex w-full items-center justify-center gap-2 rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white transition-all duration-200 hover:bg-green-700 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 active:scale-95"
